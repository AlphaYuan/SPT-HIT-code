...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, best_value2, epoch):
    ...
    if aggr_metrics['loss2'] < best_value2:
        best_value2 = aggr_metrics['loss2']
        utils.save_model(os.path.join(config['save_dir'], 'model_stage2_best_seg.pth'), epoch, val_evaluator.model_stage2)


    return aggr_metrics, best_metrics, best_value, best_value2
...
class BaseRunner(object):
    ...
    def __init__(self, model, dataloader, device, loss_module, optimizer=None, weight_mask=None,l2_reg=None, print_interval=10,
                 console=True, model_data=None,config=None,model_stage2=None,optimizer_stage2=None):
        self.model = model
...
class SupervisedRunner(BaseRunner):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.weight_cp = torch.Tensor([1, 9])
        self.weight_state = torch.Tensor([1, 1, 1, 1])
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            input_stage2 = torch.cat((X[:, :, :2*3].to(self.device), predictions[:, :, :2].detach()), dim=-1)
            predictions_stage2, _, _ = self.model_stage2(input_stage2, padding_masks)
            
            if self.config['ce_loss'] == 'ce':
                loss_ce_cp = torch.nn.CrossEntropyLoss(weight=self.weight_cp, ignore_index=5).to(self.device)
                loss_ce_state = torch.nn.CrossEntropyLoss(weight=self.weight_state, ignore_index=5).to(self.device)
            elif self.config['ce_loss'] == 'focal':
                loss_ce_cp = FocalLoss(alpha=self.weight_cp.to(self.device), ignore_index=5)
                loss_ce_state = FocalLoss(alpha=self.weight_state.to(self.device), ignore_index=5)
            
            ce_cp = loss_ce_cp(predictions_stage2[:, :, :2].permute(0, 2, 1), targets[:, :, 3].long())
            ce_state = loss_ce_state(predictions_stage2[:, :, 2:].permute(0, 2, 1), targets[:, :, 2].long())
            loss_ce = ce_cp + ce_state
            mean_loss2 = loss_ce
            batch_loss2 = loss_ce
            if self.l2_reg:
                total_loss2 = mean_loss2 + self.l2_reg * l2_reg_loss(self.model_stage2)
            else:
                total_loss2 = mean_loss2
            # Zero gradients, perform a backward pass, and update the weights.
            self.optimizer_stage2.zero_grad()
            total_loss2.backward()

            # torch.nn.utils.clip_grad_value_(self.model.parameters(), clip_value=1.0)
            torch.nn.utils.clip_grad_norm_(self.model_stage2.parameters(), max_norm=4.0)
            self.optimizer_stage2.step()

            # metrics = {"loss": mean_loss.item()}
            metrics = {"loss1": mean_loss.item(), "loss2": mean_loss2.item()}#, 'a': mse_a_mse.cpu().item(), 'k': mse_k_mse.cpu().item(), 'st': ce_state_ce.cpu().item()}
            if i % self.print_interval == 0:
                ending = "" if epoch_num is None else 'Epoch {} '.format(epoch_num)
                self.print_callback(i, metrics, prefix='Training ' + ending)
        ...
        self.epoch_metrics['epoch'] = epoch_num
        self.epoch_metrics['loss'] = epoch_loss
        self.epoch_metrics['loss2'] = epoch_loss2 / total_samples
        return self.epoch_metrics
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            ce_state = loss_ce_state(predictions_stage2[:, :, 2:].permute(0, 2, 1), targets[:, :, 2].long())
            loss_ce = ce_cp + ce_state
            mean_loss2 = loss_ce.cpu().item()
            batch_loss2 = loss_ce.cpu().item()
            #batch_loss = torch.sum(loss).cpu().item()
            ...
            metrics = {"loss1": mean_loss, 'loss2': mean_loss2}#, 'a': mse_a_mse.cpu().item(), 'k': mse_k_mse.cpu().item(), 'st': mse_state_mse.cpu().item()}
            if i % self.print_interval == 0:
                ending = "" if epoch_num is None else 'Epoch {} '.format(epoch_num)
                self.print_callback(i, metrics, prefix='Evaluating ' + ending)
...
class FocalLoss(nn.Module):
    def __init__(self, alpha=None, gamma=2.0, reduction='mean', ignore_index=5, device='cuda'):
