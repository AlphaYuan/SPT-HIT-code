...
def model_factory(config, data):
    ...
    if (task == "classification") or (task == "regression"):
        if 'model' in config and config['model'] == 'GatedLSTM':
            return GatedLSTM(input_dim=2*5, embed_dim=32, hidden_dim=64, layer_num=3)
        ...
        if 'model' in config and config['model'] == 'ModelFusion':
            return transmodel_fusion.TSTransformerEncoderClassiregressor(config['cnn_embedding'],config['whether_pos_encoding'],config['cnn_kernel'],config['cnn_outchannels'],
                                                                         config['cnn_layer'],feat_dim, max_seq_len, config['d_model'],
                                                                        config['num_heads'],
