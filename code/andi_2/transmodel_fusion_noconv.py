...
def model_factory(config, data):
    ...
    if (task == "classification") or (task == "regression"):
        if 'model' in config and config['model'] == 'GatedLSTM':
            ...
            # return GatedLSTM(input_dim=config['dimension'], embed_dim=64, hidden_dim=128, layer_num=3)
...
class AnDi_Wave(nn.Module):
    def __init__(self, input_dim, filters, kernel_size, dilation_depth):
        ...
        self.lstm = TrajModule_GatedLSTM(input_dim, filters)
...
class TrajModule_GatedLSTM(nn.Module):
    def __init__(self, embed_dim=64, hidden_dim=256, num_head=8, dropout=0.5, mlp_dim=3072, input_len=200, output_len=200) -> None:
        super().__init__()
