...
if __name__ == '__main__':
    ...
    # 回归
    config['model'] = 'ModelFusion'#'GatedLSTM'#
    ...
    config['batch_size'] = 3000#500#   #7000 is applied in the paper
    config['d_model']=32
    ...
    config['train_address'] =  '/data1/jiangy/andi_data/0629_merge/train/merge_sample_part0.csv'
    ...
    def track_data(config,filename):
        ...
        train_loader_for_weight = DataLoader(dataset=train_dataset,
                                  ...
                                  num_workers=config['num_workers'],
                                  pin_memory=True,
                                  collate_fn=lambda x: collate_fn_for_weight(x, max_len=200))
        train_weight_mask=[]
    ...
    for epoch in tqdm(range(start_epoch + 1, config["epochs"] + 1), desc='Training Epoch', leave=False):
        ...
        if config['part_data'] and (epoch % config['change_epoch'] == 0 or epoch == start_epoch + 1):
            
            data_idx = int(epoch / config['change_epoch'])
            print("change dataloader {}".format(data_idx))
            if data_idx == 12:
                exit()
            ...
            with open(os.path.join(config['train_folder'], 'dataset_part{}.pkl'.format(data_idx)), 'wb') as f:
            ...
            torch.save(t_mask, os.path.join(config['train_folder'], 'weight_part{}.pkl'.format(data_idx)))
            torch.save(train_indices, os.path.join(config['train_folder'], 'indices_part{}.pkl'.format(data_idx)))
