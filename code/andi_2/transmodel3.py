...
'''class TSTransformerEncoderClassiregressor(nn.Module):
    ...
    def __init__(self, cnn_embedding, whether_pos_encoding, cnnkernel, outputchannel_cnn, num_cnnlayer, feat_dim,
        ...
        if norm == 'LayerNorm':
            encoder_layer = TransformerEncoderLayer(hidden_dim * 8, self.n_heads, dim_feedforward, dropout * (1.0 - freeze),
    ...
    def forward(self, X, padding_masks):
        ...
        _, x = self.decoder2(x)  # x:bs, hidden_dim*8, len/8  25
        x = self.upconv1(x)  # x:bs, hidden_dim*4, len/4  50
        x = self.cat(f1, x)  # x:bs, hidden_dim*8, len/4  50
        _, x = self.decoder1(x)  # x:bs, hidden_dim*4, len/4  50
...
class TSTransformerEncoderClassiregressor(nn.Module):
    ...
    def __init__(self, cnn_embedding, whether_pos_encoding, cnnkernel, outputchannel_cnn, num_cnnlayer, feat_dim,
        ...
        if norm == 'LayerNorm':
            ...
            encoder_layer2 = TransformerEncoderLayer(hidden_dim * 4, 4, dim_feedforward, dropout * (1.0 - freeze),
                                                    activation=activation)

            encoder_layer3 = TransformerEncoderLayer(hidden_dim * 2, 4, dim_feedforward, dropout * (1.0 - freeze),
                                                    activation=activation)

            encoder_layer4 = TransformerEncoderLayer(hidden_dim, 4, dim_feedforward, dropout * (1.0 - freeze),
                                                     activation=activation)
        ...
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers)
        self.transformer_encoder2 = nn.TransformerEncoder(encoder_layer2, 3)
        self.transformer_encoder3 = nn.TransformerEncoder(encoder_layer3, 3)
        self.transformer_encoder4 = nn.TransformerEncoder(encoder_layer4, 3)
        self.projection_b = self.build_output_module(d_model,hidden_dim * 8)
        self.projection_c = self.build_output_module(hidden_dim,2)

        self.decoder2 = nn.Sequential(  # 解码器层
        ...
        self.upconv1 = nn.Sequential(nn.Upsample(scale_factor=2), nn.Conv1d(hidden_dim * 4, hidden_dim * 2, 1))
        self.decoder1 = nn.Sequential(  # 解码器层
        ...
        self.upconv0 = nn.Sequential(nn.Upsample(scale_factor=2), nn.Conv1d(hidden_dim * 2, hidden_dim, 1))
        self.decoder0 = nn.Sequential(  # 解码器层
    ...
    def forward(self, X, padding_masks):
        ...
        x =X.permute(0, 2, 1)
        x, f0 = self.encoder0(x)  # f0:bs, hidden_dim 64, len 200    x:bs, hidden_dim, len/2
        f0_tran=f0.permute(0, 2, 1)
        f0_tran = f0_tran.permute(1, 0, 2)
        f0_tran_output = self.transformer_encoder4(f0_tran)  # (seq_length, batch_size, d_model)
        f0_tran_output = self.act(f0_tran_output)  # the output transformer encoder/decoder embeddings don't include non-linearity
        f0_tran_output = f0_tran_output.permute(1, 0, 2)  # (batch_size, seq_length, d_model)
        f0_tran_output = self.dropout1(f0_tran_output)
        f0_tran_output = f0_tran_output.permute(0, 2, 1)



        x, f1 = self.encoder1(x)  # f1:bs, hidden_dim*2 128, len/2  100    x:bs, hidden_dim, len/4
        f1_tran=f1.permute(0, 2, 1)
        f1_tran = f1_tran.permute(1, 0, 2)
        f1_tran_output = self.transformer_encoder3(f1_tran)  # (seq_length, batch_size, d_model)
        f1_tran_output = self.act(f1_tran_output)  # the output transformer encoder/decoder embeddings don't include non-linearity
        f1_tran_output = f1_tran_output.permute(1, 0, 2)  # (batch_size, seq_length, d_model)
        f1_tran_output = self.dropout1(f1_tran_output)
        f1_tran_output = f1_tran_output.permute(0, 2, 1)

        x, f2 = self.encoder2(x)  # f2:bs, hidden_dim*4, 256   len/4  50  x:bs, hidden_dim, len/8
        f2_tran = f2.permute(0, 2, 1)
        f2_tran = f2_tran.permute(1, 0, 2)
        f2_tran_output = self.transformer_encoder2(f2_tran)  # (seq_length, batch_size, d_model)
        f2_tran_output = self.act(f2_tran_output)  # the output transformer encoder/decoder embeddings don't include non-linearity
        f2_tran_output = f2_tran_output.permute(1, 0, 2)  # (batch_size, seq_length, d_model)
        f2_tran_output = self.dropout1(f2_tran_output)
        f2_tran_output = f2_tran_output.permute(0, 2, 1)
        ...
        # Decoder
        x = self.upconv2(x)  # x:bs, hidden_dim*8, len/8  25
        f2_tran_output=f2_tran_output+f2
        x = self.cat(f2_tran_output, x)
        x = self.decoder2(x)  # x:bs, hidden_dim*8, len/8  25
        x = self.upconv1(x)  # x:bs, hidden_dim*4, len/4  50
        f1_tran_output=f1_tran_output+f1
        x = self.cat(f1_tran_output, x)
        x = self.decoder1(x)  # x:bs, hidden_dim*4, len/4  50
        x = self.upconv0(x)  # x:bs, hidden_dim*2, len/2  50
        f0_tran_output=f0_tran_output+f0
        x = self.cat(f0_tran_output, x)
        x = self.decoder0(x)
        x = x.permute(0, 2, 1)  # (batch_size, seq_length, d_model)
        x = x * padding_masks.unsqueeze(-1)  # zero-out padding embeddings
        #x = x.permute(0, 2, 1)  # (batch_size, seq_length, d_model)
